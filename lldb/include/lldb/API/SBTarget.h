...
public:
  ...
  /// source location in \a line_entry, including inlined instances that live
  /// in other compile units when \a check_inlines is true.
  ///
  ...
  /// file+line breakpoint: it walks each module's line tables using the same
  ...
  ///
  /// \return
  ///     A list of matching symbol contexts. Empty if no module in this
  ///     target has debug info for the requested location.
